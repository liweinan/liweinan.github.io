...
下面我们来安装OpenLDAP。openldap相关的包如下：
...
接下来是启动OpenLDAP服务。使用下面的命令启动openldap的服务：
...
可以看到OpenLDAP的服务进程名字为slapd。接下来我们可以查看一下服务器的基本配置：
...
数据库的类型默认使用openldap内置的「OpenLDAP Lightning Memory-Mapped Database」[^mdb] [^mdb2]：
