...
graph TB
    subgraph "开放标准 OIDC"
        OIDC[OIDC 核心规范]
        OIDC -->|定义| JWKS[JWKS 公钥格式]
        OIDC -->|定义| JWT[JWT 令牌结构]
    end
    subgraph "Kubernetes/OpenShift"
        K8s[实现 OIDC 身份提供商]
        K8s -->|提供| K8s_JWKS[公钥端点]
        K8s -->|签发| K8s_JWT[JWT 令牌]
    end
    subgraph "AWS"
        AWS[AWS 实现 OIDC 身份联邦]
        AWS -->|支持| IAM_OIDC[IAM OIDC 身份提供商]
        AWS -->|支持| STS_OIDC[STS AssumeRoleWithWebIdentity]
        AWS -->|验证| AWS_Validation[OIDC 令牌验证]
    end
    K8s_JWKS --> IAM_OIDC
    K8s_JWT --> STS_OIDC
