...
硬件完成初始切换后，CPU 进入 IDT 向量 **0x80** 对应的入口。x86_64 且启用 **`CONFIG_IA32_EMULATION`** 时，`idtentry_body`（[`entry_64.S`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_64.S)）里依次为：`call error_entry` → **`movq %rax,%rsp`**（用户态路径下 **`error_entry`** 末尾 **`jmp sync_regs`**，`sync_regs` **返回到** **`idtentry_body`**，不是在 `sync_regs` 内部再 `call int80_emulation`）→ **`movq %rsp,%rdi`** → **`call int80_emulation`** → **`jmp do_int80_emulation()`**：

```
CPU: INT $0x80 → IDT[0x80]，RSP ← TSS.RSP0（entry trampoline stack）
    └─ asm_int80_emulation（DECLARE_IDTENTRY_RAW，展开于 entry_64.S）
        └─ idtentry_body
              call error_entry →（用户态）jmp sync_regs()（traps.c:1044）→ 返回 ax
              movq %rax, %rsp
              movq %rsp, %rdi
              call int80_emulation（entry_64_compat.S）
                  jmp do_int80_emulation()（syscall_32.c）
```

源码入口：[`idtentry.h`](https://github.com/torvalds/linux/blob/master/arch/x86/include/asm/idtentry.h)、[`entry_64.S`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_64.S)（`idtentry_body`、`error_entry`）、[`sync_regs`](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/traps.c#L1044)、[`entry_64_compat.S`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_64_compat.S)、[`do_int80_emulation`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/syscall_32.c)。
...
[`entry_32.S`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_32.S) 注释里的 **`entry_INT80_compat`** 是对 **64 位内核上 IA32 兼容路径**的称呼；实现侧符号为 **`asm_int80_emulation`** / **`int80_emulation`** 等，并无名为 `entry_INT80_compat` 的入口符号。

### 2.4 从trampoline栈到任务栈的切换

特权级切换后，硬件帧先压在 **TSS.RSP0** 所指的 **per-CPU entry / trampoline stack**（`cpu_entry_stack`）；**从用户态**进来的 **`INT $0x80`、普通外设 IRQ、以及 IST=0 的典型异常**，在进 C 函数前多数都经同一套 **`idtentry_body` → `error_entry` → `sync_regs()`**：由 **`sync_regs()`**（[`traps.c:1044`](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/traps.c#L1044)）在必要时把 **`struct pt_regs`** 拷贝到 **`current`** 线程内核栈顶（注释亦含 **IST 栈**情形；实际栈指针调整见 **`entry_64.S`** 与 **`sync_regs`** 返回值）。**`cpu_current_top_of_stack`** 提供线程内核栈顶，供 **`syscall`** 入口等与 **`sync_regs`** 对齐。

```
idtentry_body（共用）
    └─ error_entry → jmp sync_regs()（traps.c:1044）→ 返回 pt_regs*（常为线程栈上）
```
...
下图针对 **`INT $0x80`**；从用户态进入的 **外设 IRQ** 在 **「CPU 硬件 → `error_entry`/`sync_regs`」** 一段与之相同（同一 **`TSS.RSP0`** entry stack），区别见 §3。
...
硬件中断与`int $0x80`在用户态触发时，**第一站栈相同**：均为 **`load_sp0(cpu_entry_stack(...))`** 写入的 **TSS.RSP0 → entry trampoline stack**，再经 **`idtentry_body` → `error_entry` → `sync_regs`**（与 §2 所述同源）；差异在向量、桩符号与最终 C handler。
...
**从用户态**：外设向量 **v** 经 **`irq_entries_start`** 桩 **`push`** 向量（[`idtentry.h`](https://github.com/torvalds/linux/blob/master/arch/x86/include/asm/idtentry.h)）→ **`jmp asm_common_interrupt`**，随后仍是 **`idtentry_body`**（与 **`asm_int80_emulation`** 同源）：**`call error_entry`** → **`sync_regs`** → **`movq %rax,%rsp`** → **`movq %rsp,%rdi`**，第二参数为向量（**`DEFINE_IDTENTRY_IRQ`**）→ **`call common_interrupt()`**：

```
向量 v：RSP ← TSS.RSP0（entry trampoline stack）
    └─ irq_entries_start：push v → jmp asm_common_interrupt
        └─ idtentry_body（has_error_code=1）
              call error_entry … jmp sync_regs() → 返回
              movq %rax, %rsp；movq %rsp,%rdi；（has_error_code=1）取栈上向量作第二参 → common_interrupt(regs, vector)
              call common_interrupt()（irq.c）
                  └─ call_irq_handler() → handle_irq() / apic_eoi() 等
```

参见 [`idtentry.h`](https://github.com/torvalds/linux/blob/master/arch/x86/include/asm/idtentry.h)、[`irq.c`](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/irq.c)。
...
部分 **CPU 异常**在 IDT 门中配置了 **IST≠0** 时（如 **#DF、#NMI、#MC** 等在 x86_64 Linux 中的典型 setup），硬件将 **RSP** 切到 **IST** 栈而非 **RSP0** entry stack——这与「外设 IRQ / `INT $0x80` 走 **RSP0**」并列，属于另一类交付栈。
...
各 CPU 首次执行 **`cpu_init()`** 时会写入 **`syscall`** 所用 MSR（与 **IDT 陷阱**初始化分开：`trap_init()` 更早完成异常/中断 IDT 部分）：
...
cpu_init()（arch/x86/kernel/cpu/common.c:2466）
    └─ syscall_init()（common.c:2304）
        └─ idt_syscall_init()（common.c:2268）
            └─ MSR_STAR、MSR_LSTAR(entry_SYSCALL_64)、MSR_SYSCALL_MASK（约 common.c:2268–2300）；IA32 兼容时尚有 CSTAR、SYSENTER 等
```

对应源码：[`start_kernel`](https://github.com/torvalds/linux/blob/master/init/main.c#L1017)、[`trap_init`](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/traps.c#L1682)、[`cpu_init`](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/cpu/common.c#L2466)、[`syscall_init` / `idt_syscall_init`](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/cpu/common.c#L2268-L2317)。
...
入口汇编见 [`entry_SYSCALL_64`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_64.S)。**`entry_SYSCALL_64`** 把用户栈指针暂存在 per-CPU **TSS 的 `sp2` 槽**：
