...
硬件完成初始切换后，CPU 进入 IDT 向量 **0x80** 对应的入口。x86_64 且启用 **`CONFIG_IA32_EMULATION`** 时，内核侧可概括为：

```
CPU: INT $0x80 → IDT[0x80]
    └─ asm_int80_emulation（idtentry.h DECLARE_IDTENTRY_RAW → entry_64.S 展开）
        └─ idtentry_body → error_entry（entry_64.S）
            └─ jmp sync_regs()（arch/x86/kernel/traps.c:1044）
...
[`entry_32.S`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_32.S) 注释中的 **`entry_INT80_compat`** 即指上述 x86_64 兼容路径。

### 2.4 从trampoline栈到任务栈的切换

特权级切换后，硬件帧先落在 **TSS.RSP0** 对应的 **per-CPU entry / trampoline** 栈；用户态 IDT 入口随后在汇编里进入 **`error_entry`**，再进入 **`sync_regs()`**，把 **`struct pt_regs`** 摆在当前线程内核栈上。**`cpu_current_top_of_stack`** 记录该线程内核栈顶（调度与切换路径更新），供本路径及 **`syscall`** 入口共用。

```
error_entry（entry_64.S）
    └─ sync_regs()（traps.c:1044）
```
...
|-------|-------------------|---------|
| **TSS.RSP0指向** | entry / trampoline 区域 | per-CPU entry stack |
...
```
向量号 push → jmp asm_common_interrupt（idtentry.h：irq_entries_start / idtentry_irq）
    └─ asm_common_interrupt（entry_64.S）
        └─ common_interrupt()（irq.c，DEFINE_IDTENTRY_IRQ）
...
```
syscall 指令
    └─ entry_SYSCALL_64（[`entry_64.S`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_64.S)）
```

[`entry_SYSCALL_64`](https://github.com/torvalds/linux/blob/master/arch/x86/entry/entry_64.S) 把用户栈指针暂存在 per-CPU **TSS 的 `sp2` 槽**：
