...
As we have built a knowledge base on `UriInfo` and its `UriBuilder`, now we can come back to check `ClassNode.match()` method and `SegmentNode.match()` method to see how they use the `UriInfo`. Here is the sequence diagram of `ClassNode.match()` method we have seen on above:
