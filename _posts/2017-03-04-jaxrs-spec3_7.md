...
In conclusion, the above diagram is divided into two halves: The bottom half has the `ResourceMethodRegistry` at center with some `Node` classes to do the URL path to `MethodExpression` matching work. The upper half has the `ResourceMethodInvoker` at center, to do the real method running job.

We didn't check the upper half in detail in this article, because this article is focused on the bottom half of the above diagram. In future, I'll write the article that focus on the `ResourceMethodInvoker` side to check how does RESTEasy invoke the matched resource methods.
