...
From the above diagram, we can see `SegmentNode` matching process is using the `MethodExpression` class.

In conclusion, the matching processes are mainly in `ClassNode` and `SegmentNode`. `ClassNode` deals with class matching process, and `SegmentNode` processes the method matching process. With this information, we can check the codes with spec.
