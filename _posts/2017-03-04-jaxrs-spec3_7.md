...
We can see the `UriInfo` interface is a standard `javax.ws.rs.core` class, which means this is a class defined by the SPEC. The `ResteasyUriInfo` is an implementation of this `UriInfo` interface. Here is the javadoc in `UriInfo` that describes the purpose of the interface:

>  An injectable interface that provides access to application and request URI information.

Besides the meaning of encapsulate URI info, it also defines many methods that deals with URI info.
