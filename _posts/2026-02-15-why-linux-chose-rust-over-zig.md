...
abstract: "2022年12月，Linux 6.1正式发布，首次将Rust作为内核的第二种编程语言。本文深入分析了Linux内核选择Rust而非Zig的核心原因，包括时机、语言特性差异、社区生态等多个维度，并探讨了两种语言在系统编程领域的不同定位。"
---
...
最直接的原因可以概括为：**当内核在2022年底正式引入Rust时，Zig还没准备好；而当Zig逐渐成熟时，内核的"第二语言"席位已经被Rust占据**。2022年10月，Linus Torvalds将Rust代码合并到Linux 6.1的开发周期中[^11]，同年12月11日，Linux 6.1正式发布[^12]，Rust成为Linux历史上首次被接纳的C语言之外的编程语言[^13]。
...
Rust for Linux项目于2020年在Linux内核邮件列表中宣布启动[^1]，经过两年多的开发，于2022年12月随Linux 6.1正式进入稳定版本。在2019-2020年内核讨论引入第二语言时，Zig（2015年诞生）还处于早期阶段，而Rust背后有**Mozilla、微软、谷歌**等巨头的投入。到2025年12月，Rust已正式从"实验性"状态转为Linux内核的核心组成部分[^2]。
...
C++的异常处理会引入非局部控制流跳转，这在需要绝对确定性的内核代码中是不可接受的。C语言的错误返回值机制虽然繁琐，但路径清晰、透明可控。
...
Linus指出："任何喜欢在背后隐藏内存分配等操作的编译器或语言，都不是内核开发的好选择。"[^15]内核需要对每一个字节的内存分配有完全的控制权。
...
Linus认为："你可以用C编写面向对象的代码（对文件系统等很有用），而不需要C++中的那些垃圾。"[^15]Linux内核用C语言的结构体和函数指针实现了充分的面向对象设计。
...
[^11]: [The Initial Rust Infrastructure Has Been Merged Into Linux 6.1](https://www.phoronix.com/news/Rust-Is-Merged-Linux-6.1) - Phoronix, 2022年10月报道Rust合并到Linux 6.1

[^12]: [Linus Torvalds reveals Linux kernel 6.1](https://www.theregister.com/2022/12/11/linux_6_1/) - The Register, 2022年12月11日报道Linux 6.1发布

[^13]: [Linux 6.1 Officially Adds Support for Rust in the Kernel](https://www.infoq.com/news/2022/12/linux-6-1-rust/) - InfoQ关于Linux 6.1正式添加Rust支持的详细报道

[^14]: [Re: Compiling C++ kernel module + Makefile](https://harmful.cat-v.org/software/c++/linus) - Linus Torvalds, 2004年1月19日在Linux内核邮件列表的回复

[^15]: [Re: [RFC] Convert builtin-mailinfo.c to use The Better String Library](https://lwn.net/Articles/249460/) - Linus Torvalds, 2007年9月6日在Git邮件列表关于C++的完整论述

[^16]: [Low-cost deterministic C++ exceptions for embedded systems](https://www.research.ed.ac.uk/files/78829292/low_cost_deterministic_C_exceptions_for_embedded_systems.pdf) - University of Edinburgh, 2019年ACM编译器构造国际会议论文

[^17]: [Propagating C++ exceptions across the user/kernel boundary](https://doi.org/10.1145/3764860.3768332) - Voronetskiy & Spink, University of St Andrews, PLOS 2025
